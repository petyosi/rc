...
- Parent and child engine forwarding creates separate engine-local cycles with the same transaction ID. Forwarding uses an `origin` field and does not invent a parent relationship.
...
If a projection, comparator, or subscriber throws, the current cycle is marked `aborted`. Diagnostics are finalized and delivered, then the original thrown value continues to the publisher unchanged. Observer errors are isolated and cannot replace the application result or error.
