...
- Memoize observers and redactors when practical. A changed observer or option replaces the subscription for future cycles.
...
Value capture defaults to `none`. Start there, add node labels, and enable `summary` only where values help. Always redact before sending records outside the process. See the core [Propagation Diagnostics](../../reactive-engine-core/docs/06.diagnostics.md) guide for the event model and troubleshooting procedure.
